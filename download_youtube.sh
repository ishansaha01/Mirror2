...
if [ ! -f "requirements.txt" ]; then
  echo -e "pytubefix>=2.1.0\ndatetime" > requirements.txt
fi

# Create the downloader script if it doesn't exist
if [ ! -f "youtube_downloader.py" ]; then
  cat > youtube_downloader.py << 'EOF'
#!/usr/bin/env python3
...
def download_video(url, output_path=None):
    ...
    if not output_path:
        output_path = os.path.join(os.getcwd(), "dataSets", "test_data", "results")
    try:
...
# Run the downloader
python youtube_downloader.py "$1" $OUTPUT_ARG
