...
def map_frame_to_datapoint(frame_number, video_frame_count, eventfulness_length):
    ...
    eventfulness_index = min(int(frame_number / ratio),
                             eventfulness_length - 1)
    ...
    try:
        for item in sorted(os.listdir(directory)):
            ...
            if os.path.isdir(item_path):
                items.append(
                    {"name": item, "type": "directory", "path": item_path})
        ...
        for root, _, files in os.walk(directory):
            for file in sorted(files):
                if file.lower().endswith('.mp4'):
                    ...
                    display_name = file if rel_path == '.' else os.path.join(
                        rel_path, file)
                    items.append({
                        "name": display_name,
                        "type": "video",
    ...
    if cap.isOpened():
        ...
        if default_config and "eventfulness" in default_config and len(default_config["eventfulness"]) > 0:
            default_eventfulness_data = {
                # First dimension for visualization
                "data": default_config["eventfulness"][0],
                # Full eventfulness vectors (all dimensions)
...
app.layout = html.Div([
    ...
    dcc.Store(id='current-directory',
              data="/home/is1893/Mirror2/dataSets/test_data/val/"),
    ...
    # Store for detected peaks and dance steps
    dcc.Store(id='peak-data', data=None),
    # Store for extracted frames at peaks
    dcc.Store(id='peak-frames', data=None),
    # Store for sliding window frames
    dcc.Store(id='sliding-window-frames', data=None),
    # Store for cluster assignments
    dcc.Store(id='cluster-assignments', data=None),
    # Store for sequence description
    dcc.Store(id='sequence-description', data=None),
    dcc.Store(id='pca-results', data=None),  # Store for PCA analysis results
    # Store for full video pose estimation data
    dcc.Store(id='full-video-pose-data', data=None),
    # Store for cluster centroids
    dcc.Store(id='cluster-centroids', data=None),
    # Store for cosine similarity data
    dcc.Store(id='cosine-similarity-data', data=None),
    # Store for time series segmentation data
    dcc.Store(id='segmentation-data', data=None),
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                # Cosine similarity graph section
                html.Div([
                    html.Div([
                        html.H3("Cosine Similarity to Cluster Centroids",
                                style={'display': 'inline-block'}),
                        html.Div([
                            html.Button(
                                "Download CSV",
                                id="download-cosine-similarity-button",
                                style={
                                    'marginLeft': '20px',
                                    'padding': '5px 10px',
                            ...
                            dcc.Download(id="download-cosine-similarity-csv")
                        ], style={'display': 'inline-block', 'float': 'right', 'marginTop': '5px'})
                ...
                ], id='cosine-similarity-section', style={'display': 'none'}),

                # Time series segmentation section
                html.Div([
                    html.H3("Wavelet-Based Time Series Segmentation"),
                    html.Div([
                        html.Div([
                            html.Label("Vector Type:", style={
                                       'marginRight': '10px'}),
                            dcc.Dropdown(
                                id='segmentation-vector-type',
                                options=[
                                    {'label': 'Eventfulness Vector',
                                        'value': 'eventfulness'},
                                    {'label': 'Pose Vector', 'value': 'pose'},
                                    {'label': 'Combined Vectors',
                                        'value': 'combined'}
                                ],
                                value='pose',
                                style={
                                    'width': '200px', 'display': 'inline-block', 'marginRight': '20px'}
                            ),
                            html.Label("Segmentation Method:", style={
                                       'marginRight': '10px'}),
                            dcc.RadioItems(
                                id='segmentation-method',
                                options=[
                                    {'label': 'Wavelet-based', 'value': 'wavelet'},
                                    {'label': 'Periodicity-based',
                                        'value': 'periodicity'}
                                ],
                                value='wavelet',
                                labelStyle={
                            ...
                            html.Label("Threshold Factor:", style={
                                       'marginRight': '10px'}),
                            dcc.Slider(
                                id='segmentation-error-threshold',
                                min=0.01,
                                max=0.2,
                                step=0.01,
                                value=0.05,
                                marks={
                                    i/100: f'{i/100:.2f}' for i in range(1, 21, 4)},
                                tooltip={"placement": "bottom",
                                         "always_visible": True}
                            ),
                            html.Label("Min Segment Length:", style={
                                       'marginRight': '10px', 'marginLeft': '20px'}),
                            dcc.Input(
                                id='segmentation-min-length',
                                type='number',
                                min=3,
                                max=50,
                                step=1,
                            ...
                            html.Button("Run Segmentation", id='segmentation-button', n_clicks=0, style={
                                'marginLeft': '20px',
                                'padding': '8px 16px',
                        ...
                        html.Div([
                            html.P("Choose between wavelet-based segmentation (identifies energy changes at multiple scales) or periodicity-based segmentation (identifies changes in periodic patterns).",
                                 style={'fontStyle': 'italic', 'color': '#666', 'fontSize': '0.9em', 'marginTop': '5px'})
                        ]),
                    ]),
                    dcc.Graph(
                ...
                ], id='segmentation-section', style={'display': 'none', 'marginTop': '20px'}),
                ...
                html.Div([
                    ...
                    html.Div([
                        html.Div([
                            html.Div("Next Peak:", style={
                                     'fontWeight': 'bold', 'display': 'inline-block', 'marginRight': '10px'}),
                            html.Div(
                                id='next-peak-time', style={'display': 'inline-block', 'fontSize': '1.2em'})
                        ]),
                        html.Div([
                            html.Div("Peak Value:", style={
                                     'fontWeight': 'bold', 'display': 'inline-block', 'marginRight': '10px'}),
                            html.Div(
                                id='peak-value', style={'display': 'inline-block', 'marginRight': '10px'}),
                ...
                html.Div([
                    ...
                    html.Div([
                        html.Div([
                            html.Div("Frames extracted at detected peaks:", style={
                                     'marginBottom': '10px', 'display': 'inline-block', 'marginRight': '20px'}),
                            ...
                            html.Button("Run Full Video Analysis", id='run-full-pose-btn', n_clicks=0, style={
                                'marginBottom': '10px',
                                'padding': '8px 16px',
                        ...
                        html.Div([
                            ...
                            html.Label("Max Number of Clusters:",
                            ...
                            dcc.Input(
                                ...
                                max=40,
                                style={
                                    'width': '80px', 'display': 'inline-block', 'marginRight': '20px'}
                            ),
                        ], id='cluster-controls', style={'marginBottom': '10px', 'display': 'block'}),
                        html.Div(
                            id='cluster-info', style={'marginBottom': '10px', 'fontSize': '0.9em', 'color': '#666'}),
                        html.Div([
                            ...
                            html.H4("Silhouette Score Analysis",
                        ...
                        html.Div([
                            html.H4("Video Sequence Description",
                                    className="section-header"),
                            html.Button("Generate Sequence Description", id='generate-sequence-btn', n_clicks=0,
                                        style={'marginBottom': '10px'}),
                            html.Div(id='sequence-description-display',
                                     style={'whiteSpace': 'pre-wrap'}),
                            dcc.Graph(
                                id='sequence-timeline', style={'marginTop': '15px', 'height': '250px'})
                        ...
                        html.Div([
                            ...
                            html.Div([
                                html.Label("PCA Components:", style={
                                           'marginRight': '10px'}),
                                html.Div([
                                    dcc.Slider(
                            ...
                            html.Div(
                                id='pca-info', style={'marginBottom': '10px', 'fontSize': '0.9em', 'color': '#666'}),
                            dcc.Graph(
                        ...
                        html.Div(
                            id='peak-frames-gallery', style={'display': 'flex', 'flexWrap': 'wrap', 'gap': '10px'}),
                    ], style={'border': '1px solid #ddd', 'borderRadius': '5px', 'padding': '15px', 'backgroundColor': '#f9f9f9'})
...
@callback(
    ...
    # Also show the peak frames section
    Output('peak-frames-section', 'style'),
    # Also show the sliding window section
    Output('sliding-window-section', 'style'),
    # Extract frames for sliding window
    Output('sliding-window-frames', 'data'),
    Input('current-video', 'data'),
...
def update_eventfulness_graph(eventfulness_data, video_info, current_video):
    ...
    peak_frames = extract_frames_at_peaks(
        current_video, peaks, video_info, eventfulness_data)

    # Find the next upcoming peak
    next_peak, distance_to_peak, next_peak_value = find_next_maximum(
        data, initial_index, peaks)

    # Log the initial graph setup and peak detection
    logger.info(f"Creating eventfulness graph with {len(data)} data points")
    logger.info(
        f"Initial marker position: index={initial_index}, value={initial_value:.3f}")
    ...
    if next_peak is not None:
        logger.info(
            f"Next peak at index {next_peak}, distance: {distance_to_peak}, value: {next_peak_value:.3f}")
    ...
    if next_peak is not None:
        fig.add_trace(go.Scatter(
            ...
            hoverinfo='text',
            hovertext=[
                f"Next peak: {next_peak_value:.3f}, Distance: {distance_to_peak}"],
            visible=True
        ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            # Approximate 1s before, 3s after
            range=[max(0, initial_index - 30), initial_index + 90],
        ),
        yaxis=dict(
            title="Eventfulness",
            showgrid=True,
...
def log_video_events(playing, current_time, seek_to, current_debug):
    ...
    if trigger_id == 'video-player':
        if trigger_prop == 'playing':
            ...
            # Add new line at the top, keep only last 20
...
@callback(
    ...
    # Also control the sliding window interval
    Output('sliding-window-update-interval', 'disabled'),
...
def update_graph_marker(n_intervals, current_time, playing, video_info, eventfulness_data, current_debug, current_figure):
    ...
    data = eventfulness_data['data']
    data_index = map_frame_to_datapoint(
        current_frame, video_info['frame_count'], len(data))
    ...
    if len(updated_figure['data']) > 1:
        ...
        updated_figure['data'][1]['hovertext'] = [
            f"Peak: {val:.3f}" for val in peak_values]
    ...
    if len(updated_figure['data']) > 4 and next_peak is not None:
        ...
        updated_figure['data'][4]['hovertext'] = [
            f"Next peak: {next_peak_value:.3f}, Time: {time_to_peak:.2f}s"]
        updated_figure['data'][4]['visible'] = True

    # Verify that the marker was updated correctly
    is_valid = verify_marker_position(
        updated_figure, data_index, current_value)
    ...
    if next_peak is not None:
        ...
        for i in range(num_bars):
            # Increasing heights: 10px, 12px, 14px, etc.
            bar_height = 10 + (i * 2)
...
def update_sliding_window(n_intervals, current_time, video_info, peak_frames):
    ...
    if n_intervals % 10 == 0:
        timestamp = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]
        logger.info(
            f"[{timestamp}] Updating sliding window at time: {current_time:.3f}s")
    ...
    if n_intervals % 30 == 0:
        logger.info(
            f"Dynamic preview time: {preview_time:.2f}s (based on {num_peaks} peaks in {duration:.2f}s video)")
    ...
    if n_intervals % 10 == 0:
        logger.info(
            f"Found {len(relevant_frames)} relevant peak frames in time window")
    ...
    for i, frame_data in enumerate(relevant_frames):
        ...
        if time_diff < -0.2:
            ...
            z_index = 1000 - int(time_diff * 100)
            animation = ""
            label = ""
            label_style = {}
            ...
            position_percent = 90 - \
                ((preview_time - time_diff) / preview_time) * 40
            ...
            # Higher for frames closer to current time
            z_index = 1000 - int(time_diff * 100)
            animation = ""
    ...
    logger.info(
        f"[{timestamp}] Updated sliding window with {len(frame_elements)} peak frames")
...
def handle_clustering(n_clicks, vector_type, peak_frames, max_clusters, existing_assignments):
    ...
    if trigger_id == 'cluster-button' and n_clicks > 0:
        ...
        if 'k_values' in cluster_info and 'silhouette_scores' in cluster_info:
            cluster_assignments['k_values_data'] = json.dumps(
                cluster_info['k_values'])
            cluster_assignments['silhouette_scores_data'] = json.dumps(
                cluster_info['silhouette_scores'])
            cluster_assignments['penalized_scores_data'] = json.dumps(
                cluster_info['penalized_scores'])
            cluster_assignments['best_k_value'] = str(
        ...
        info_div = html.Div([
            html.Div(info_text, style={
                     'color': '#2196F3', 'fontWeight': 'bold', 'marginBottom': '10px'}),
            html.Div("Silhouette scores by cluster count:", style={
                     'fontSize': '0.9em', 'marginTop': '5px'}),
            html.Div([
    ...
    if trigger_id == 'vector-type':
        viz_style = {'display': 'block'} if existing_assignments else {
            'display': 'none'}
...
def generate_sequence_description(peak_frames, cluster_assignments):
    ...
    sequence_text = "Cluster sequence: " + \
    ...
    most_common_pct = (most_common_count / total_steps *
...
def update_peak_frames_gallery(peak_frames, cluster_assignments, frame_view):
    ...
    if cluster_assignments:
        ...
        for peak_idx in peak_frames.keys():
            cluster_id = cluster_assignments.get(
                str(peak_idx), -999)  # -999 for unclustered
        ...
        sorted_cluster_ids = sorted(
            [c for c in cluster_groups.keys() if c != -999])
        ...
        for cluster_id in sorted_cluster_ids:
            ...
            cluster_peaks_sorted = sorted(
                cluster_peaks, key=lambda x: peak_frames[x]['time'])
    ...
    for peak_idx in sorted_peaks:
        ...
        if vector_magnitude is not None:
            info_divs.append(
                html.Div(f"Eventfulness Mag: {vector_magnitude:.3f}", style={
                         'fontSize': '0.8em', 'fontWeight': 'bold', 'color': '#2196F3'})
            )

        # Add pose information if available
        if pose_detected:
            info_divs.append(
                html.Div(f"Pose Detected ✓", style={
                         'fontSize': '0.8em', 'fontWeight': 'bold', 'color': 'green'})
            )
            if pose_magnitude is not None:
                info_divs.append(
                    html.Div(f"Pose Mag: {pose_magnitude:.3f}", style={
                             'fontSize': '0.8em', 'color': 'green'})
                )
        else:
            info_divs.append(
                html.Div("No Pose Detected", style={
        ...
        thumbnail = html.Div([
            html.Img(src=frame_url, style={
                     'width': '150px', 'height': 'auto', 'objectFit': 'cover'}),
            html.Div(info_divs, style={
...
def handle_sequence_description(n_clicks, peak_frames, cluster_assignments):
    ...
    sequence_data = generate_sequence_description(
        peak_frames, cluster_assignments)
...
def update_cluster_visualization(peak_frames, cluster_assignments, vector_type):
    ...
    try:
        ...
        if X_scaled.shape[1] > 50:
            from sklearn.decomposition import PCA
            # Make sure n_components doesn't exceed the number of samples or features
        ...
        # Apply t-SNE if we have enough samples, otherwise use PCA directly
        if len(vectors) >= 3:  # t-SNE needs at least 3 samples
            # Ensure perplexity is valid
            perplexity = min(30, max(1, len(vectors) - 1))
            tsne = TSNE(n_components=2, random_state=42, perplexity=perplexity)
            X_2d = tsne.fit_transform(X_reduced)
        else:
            # Not enough samples for t-SNE, use PCA directly
        ...
        # Fallback to PCA
        from sklearn.decomposition import PCA
        # Use a simpler approach for PCA fallback
        n_components = min(2, X_scaled.shape[0], X_scaled.shape[1])
        pca = PCA(n_components=n_components)
        X_2d = pca.fit_transform(X_scaled)
        # If we only have 1 component, add a zero column to make it 2D
        if X_2d.shape[1] == 1:
            X_2d = np.column_stack([X_2d, np.zeros(X_2d.shape[0])])
    ...
    for cluster_id in unique_clusters:
        ...
        cluster_mask = [i for i, c in enumerate(
            cluster_labels) if c == cluster_id]
        ...
        for i in cluster_mask:
            ...
            if vector_type == 'pose' or vector_type == 'combined':
                if 'pose_vector' in frame_info and frame_info['pose_vector'] is not None:
                    ...
                    hover_info.append(
                        f"Pose Detected: {frame_info.get('pose_detected', False)}")
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                size=size,
                symbol=symbol,
                line=dict(width=1, color='white') if cluster_id != -
                1 else dict(width=0)
            ),
            text=hover_texts,
            hoverinfo='text',
    ...
    if 'silhouette_scores_data' in cluster_assignments:
        silhouette_scores = json.loads(
            cluster_assignments['silhouette_scores_data'])
    if 'penalized_scores_data' in cluster_assignments:
        penalized_scores = json.loads(
            cluster_assignments['penalized_scores_data'])
...
def handle_pca_analysis(n_clicks, vector_type, peak_frames, n_components):
    ...
    pca_fig.update_layout(
        ...
        xaxis=dict(
            title=f"PC1 ({pca_results['explained_variance_ratio'][0]:.2%} variance)"),
        yaxis=dict(
            title=f"PC2 ({pca_results['explained_variance_ratio'][1]:.2%} variance)"),
        hovermode='closest',
        plot_bgcolor='white',
        width=800,
        height=500
...
def perform_pca_analysis(vectors, n_components=0.95):
    ...
    logger.info(
        f"PCA analysis completed: {len(vectors)} vectors reduced to {pca.n_components_} components")
    logger.info(
        f"Total explained variance: {sum(pca.explained_variance_ratio_):.4f}")
...
# Import PyWavelets at the top of the file with other imports

# Function to find dominant frequencies in time series data


def find_dominant_frequencies(data, sampling_rate=60):  # Assuming 60 fps
    """
    Find dominant frequencies in the time series data using FFT

    Args:
        data: Numpy array of time series data (samples x features)
        sampling_rate: Sampling rate in Hz (default: 60 fps)

    Returns:
        List of dominant frequencies for each feature
    """
    ...
    for i in range(data.shape[1]):
        ...
        peaks, _ = find_peaks(yf_pos, height=0.1*np.max(yf_pos))

        # Sort peaks by amplitude
        peak_amplitudes = yf_pos[peaks]
        sorted_idx = np.argsort(peak_amplitudes)[::-1]  # Descending order
        # Top 3 peaks or all if fewer
        top_peaks = peaks[sorted_idx[:3] if len(
            sorted_idx) >= 3 else sorted_idx]

        # Store the dominant frequencies
        dominant_freqs.append(xf_pos[top_peaks] if len(top_peaks) > 0 else [])

    return dominant_freqs

# Function for sliding window change detection


def sliding_window_change_detection(data, window_size=10, overlap=0.5, threshold=2.0):
    """
    Detect change points using sliding window statistics

    Args:
        data: Numpy array of time series data (samples x features)
        window_size: Size of the sliding window
        overlap: Fraction of window to overlap
        threshold: Threshold for change detection (in std deviations)
    ...
    for start in range(0, n_samples - window_size, step_size):
        ...
        window_stats.append((mean, std))
        window_times.append(start + window_size // 2)  # Center of window

    # Detect changes between consecutive windows
    change_scores = []

    for i in range(1, len(window_stats)):
        prev_mean, prev_std = window_stats[i-1]
        curr_mean, curr_std = window_stats[i]

        # Calculate normalized distance between distributions
        # Using a simplified Bhattacharyya distance
        mean_diff = np.sum((prev_mean - curr_mean)**2)
        std_diff = np.sum((prev_std - curr_std)**2)

        # Combined score
        score = mean_diff + std_diff
        change_scores.append((window_times[i], score))

    # Find significant changes
    scores = np.array([score for _, score in change_scores])
    mean_score = np.mean(scores)
    std_score = np.std(scores)

    change_points = []
    for time, score in change_scores:
...
def create_segments_from_change_points(data, change_points, min_segment_size=5):
    """
    Create segments from detected change points

    Args:
        data: Numpy array of time series data
        change_points: List of detected change points
        min_segment_size: Minimum size of a segment
    ...
    all_points = [0] + change_points + [n_samples-1]
    all_points = sorted(list(set(all_points)))  # Remove duplicates

    # Filter points to ensure minimum segment size
    filtered_points = [all_points[0]]  # Always include the start point

    for i in range(1, len(all_points)):
        if all_points[i] - filtered_points[-1] >= min_segment_size:
            filtered_points.append(all_points[i])

    return filtered_points

# Function to perform periodicity-based segmentation


def periodicity_based_segmentation(time_series, window_size=15, overlap=0.5, threshold=1.5, min_segment_size=5):
    """
    Performs periodicity-based segmentation on time series data.

    Args:
        time_series: Numpy array of time series data (samples x features)
        window_size: Size of the sliding window
        overlap: Fraction of window to overlap
    ...
    Returns:
        List of segment boundaries
    """
    # Detect change points using sliding window
    change_points = sliding_window_change_detection(
        time_series,
    ...
    segment_boundaries = create_segments_from_change_points(
        time_series,
        change_points,
...
# Function to smooth time series data using moving average


def smooth_time_series(time_series, window_size=5):
    """
    Applies moving average smoothing to time series data.

    Args:
        time_series: Numpy array of time series data points
    ...
    if len(time_series.shape) > 1:
        ...
        smoothed_data = np.zeros_like(time_series)
        for i in range(time_series.shape[1]):
            # Create a pandas Series for easy moving average calculation
            series = pd.Series(time_series[:, i])
            # Apply rolling mean (moving average) with center=True to avoid phase shift
            smoothed_data[:, i] = series.rolling(window=window_size, center=True).mean(
            ).fillna(method='bfill').fillna(method='ffill').values
        return smoothed_data
    else:
        # 1D case
        series = pd.Series(time_series)
        return series.rolling(window=window_size, center=True).mean().fillna(method='bfill').fillna(method='ffill').values

# Function to perform wavelet-based segmentation on time series data


def wavelet_based_segmentation(time_series, wavelet='db4', level=3, threshold_factor=1.5, min_segment_length=5):
    """
    Implements a wavelet-based segmentation algorithm for time series data.

    The algorithm uses wavelet transform to identify changes in the signal's
    frequency characteristics, which often indicate boundaries between different
    behaviors or patterns.

    Args:
        time_series: List or numpy array of time series data points
        wavelet: Wavelet type to use (default: 'db4')
        level: Decomposition level (default: 3)
        threshold_factor: Factor to multiply std dev for thresholding (default: 1.5)
        min_segment_length: Minimum length of resulting segments (default: 5)

    Returns:
        List of segment boundaries (indices where segments begin)
    """
    import numpy as np
    from scipy.signal import find_peaks
    ...
    # Ensure level is valid for the data length
    data_length = len(time_series)
    max_level = pywt.dwt_max_level(data_length, pywt.Wavelet(wavelet).dec_len)
    level = min(level, max_level)

    # If level is 0, we can't do wavelet decomposition
    if level == 0:
    ...
    # Smooth the time series data first
    time_series_smoothed = smooth_time_series(time_series, window_size=5)

    # For multidimensional time series, calculate wavelet energy for each dimension
    # and then combine them
    if len(time_series_smoothed.shape) > 1:
        n_samples = time_series_smoothed.shape[0]
        n_dimensions = time_series_smoothed.shape[1]

        # Initialize combined energy signal
        combined_energy = np.zeros(n_samples)

        # Process each dimension
        for dim in range(n_dimensions):
            # Extract this dimension
            signal = time_series_smoothed[:, dim]

            # Perform wavelet decomposition
            coeffs = pywt.wavedec(signal, wavelet, level=level)

            # Calculate wavelet energy (skip approximation coefficients)
            energy = np.zeros(n_samples)
            for j, coeff in enumerate(coeffs[1:], 1):  # Skip approximation
                # Reconstruct detail coefficients at original length
                reconstructed = pywt.upcoef(
                    'd', coeff, wavelet, level=j, take=n_samples)
                energy += reconstructed**2

            # Add to combined energy
            combined_energy += energy
    else:
        # 1D case - simpler processing
        n_samples = len(time_series_smoothed)

        # Perform wavelet decomposition
        coeffs = pywt.wavedec(time_series_smoothed, wavelet, level=level)
        ...
        for j, coeff in enumerate(coeffs[1:], 1):  # Skip approximation
            reconstructed = pywt.upcoef(
                'd', coeff, wavelet, level=j, take=n_samples)
            combined_energy += reconstructed**2

    # Normalize energy
    if np.max(combined_energy) > 0:
        combined_energy = combined_energy / np.max(combined_energy)

    # Calculate the absolute difference of energy to detect changes
    energy_diff = np.abs(np.diff(combined_energy))
    ...
    # Find indices where energy difference exceeds threshold
    change_points = np.where(energy_diff > threshold)[0]

    # Filter change points to ensure minimum segment length
    filtered_change_points = [0]  # Always include the start point

    for cp in change_points:
        # Only add if it's far enough from the last change point
        if cp - filtered_change_points[-1] >= min_segment_length:
            # +1 because diff reduces length by 1
            filtered_change_points.append(cp + 1)

    # Always include the end point
    if n_samples - 1 not in filtered_change_points and n_samples - filtered_change_points[-1] >= min_segment_length:
        filtered_change_points.append(n_samples)

    return filtered_change_points

# Function to apply wavelet-based segmentation to vector time series


def segment_vector_time_series(vector_data, vector_type='pose', error_threshold=0.05, min_segment_length=5, segmentation_method='wavelet'):
    """
    Applies segmentation to vector time series data using either wavelet or periodicity-based methods.

    Args:
        vector_data: Dictionary containing frame data with vectors
        vector_type: Type of vector to use ('eventfulness', 'pose', or 'combined')
        error_threshold: Threshold factor for change detection
        segmentation_method: Method to use ('wavelet' or 'periodicity')
        min_segment_length: Minimum length of resulting segments

    Returns:
        Dictionary with segment boundaries and segment information
    """
    import numpy as np
    ...
    # Extract frame numbers and vectors
    frame_numbers = []
    vectors = []

    # Sort frames by frame number
    sorted_frames = sorted(vector_data.items(), key=lambda x: int(
        x[0]) if x[0].isdigit() else float('inf'))

    for frame_idx, frame_data in sorted_frames:
        if vector_type == 'eventfulness' and 'eventfulness_vector' in frame_data:
            frame_numbers.append(frame_data.get(
                'frame_number', int(frame_idx)))
            vectors.append(frame_data['eventfulness_vector'])
        elif vector_type == 'pose' and 'pose_vector' in frame_data:
            frame_numbers.append(frame_data.get(
                'frame_number', int(frame_idx)))
            vectors.append(frame_data['pose_vector'])
        elif vector_type == 'combined':
            # Combine eventfulness and pose vectors if both are available
            ev = frame_data.get('eventfulness_vector', [])
            pv = frame_data.get('pose_vector', [])
            if ev and pv:
                frame_numbers.append(frame_data.get(
                    'frame_number', int(frame_idx)))
                vectors.append(np.concatenate([ev, pv]))

    if len(vectors) < 2:
        return {
            'segment_boundaries': [0],
            'frame_boundaries': [frame_numbers[0]] if frame_numbers else [],
            'segments': []
        }
    ...
    # Apply wavelet-based segmentation
    # Convert error_threshold to threshold_factor for wavelet segmentation
    # Scale appropriately for wavelet energy
    threshold_factor = error_threshold * 30

    # Determine appropriate wavelet level based on data length
    data_length = len(vectors)

    # Apply segmentation based on selected method
    if segmentation_method == 'periodicity':
        # Apply periodicity-based segmentation
        logger.info(
            f"Using periodicity-based segmentation with threshold={error_threshold}")

        try:
            # Convert error_threshold to appropriate threshold for periodicity detection
            # The periodicity method uses a different scale
            threshold = error_threshold * 30

            # Determine window size based on data length (about 10% of data)
            window_size = max(min_segment_length, min(
                30, int(data_length * 0.1)))

            segment_boundaries = periodicity_based_segmentation(
                vectors,
                window_size=window_size,
                overlap=0.5,
                threshold=threshold,
                min_segment_size=min_segment_length
            )
        except Exception as e:
            logger.error(
                f"Error in periodicity-based segmentation: {str(e)}. Using simple segmentation.")
            segment_boundaries = [0, data_length]  # Fallback to single segment
    else:
        # Apply wavelet-based segmentation (default)
        # Make sure we have enough data for wavelet decomposition
        if data_length < 4:  # Need at least a few points for meaningful segmentation
            # Not enough data for wavelet decomposition, just return start and end
            segment_boundaries = [0]
            if data_length > 0:
                segment_boundaries.append(data_length)

            logger.info(
                f"Not enough data for wavelet decomposition. Using simple segmentation.")
        else:
            # Choose wavelet level based on data length
            if data_length < 32:
                wavelet_level = 1
            elif data_length < 64:
                wavelet_level = 2
            ...
            try:
                segment_boundaries = wavelet_based_segmentation(
                    vectors,
                    wavelet='db4',  # Daubechies 4 wavelet works well for most signals
                    level=wavelet_level,
                ...
                logger.error(
                    f"Error in wavelet segmentation: {str(e)}. Using simple segmentation.")
                # Fallback to single segment
                segment_boundaries = [0, data_length]

    # Map segment boundaries to frame numbers
    frame_boundaries = [frame_numbers[i] if i < len(frame_numbers) else frame_numbers[-1]
    ...
    for i in range(len(segment_boundaries) - 1):
        ...
        # Calculate mean vector for the segment
        segment_vectors = vectors[start_idx:end_idx]
        mean_vector = np.mean(segment_vectors, axis=0)
        ...
        segments.append({
            ...
            'end_frame': frame_numbers[end_idx-1] if end_idx-1 < len(frame_numbers) else frame_numbers[-1],
            'duration': duration,
...
def cluster_eventfulness_vectors(peak_frames, max_clusters=6, vector_type='eventfulness'):
    ...
    if len(vectors) < 2:
        logger.warning(
            f"Not enough {vector_type} vectors for clustering (need at least 2)")
        return None, None
    ...
    logger.info(
        f"Standardized {len(vectors)} {vector_type} vectors before clustering")
    ...
    # Adjust this value to control how much to penalize larger cluster counts
    penalty_factor = 0.00
    penalized_scores = [score - (k * penalty_factor)
                        for score, k in zip(silhouette_scores, k_values)]
...
def extract_frames_at_peaks(video_path, peaks, video_info, eventfulness_data):
    ...
    frames_dir = os.path.join(
        RESULTS_DIR, f"peak_frames_{video_filename}_{timestamp}")
    ...
    for i, peak_idx in enumerate(peaks):
        ...
        if ret:
            ...
            annotated_frame, pose_vector, pose_success = perform_pose_estimation(
                frame)

            # Save the original frame
            frame_path = os.path.join(
                frames_dir, f"peak_{i}_idx_{peak_idx}_frame_{frame_number}.jpg")
            cv2.imwrite(frame_path, frame)

            # Save the annotated frame with pose landmarks
            annotated_frame_path = os.path.join(
                frames_dir, f"pose_annotated_peak_{i}_idx_{peak_idx}_frame_{frame_number}.jpg")
            ...
            if full_vectors and peak_idx < len(full_vectors[0]):
                ...
                eventfulness_vector = [full_vectors[dim][peak_idx]
                                       for dim in range(len(full_vectors))]
                logger.info(
                    f"Extracted full eventfulness vector for peak {i}: index={peak_idx}, vector_length={len(eventfulness_vector)}")
            else:
                logger.warning(
                    f"Full eventfulness vectors not available for peak {i}: index={peak_idx}")
            ...
            logger.info(
                f"Extracted frame at peak {i}: index={peak_idx}, frame={frame_number}, time={frame_number/fps:.2f}s, pose_detected={pose_success}")
        else:
            logger.error(
                f"Failed to extract frame at peak {i}: index={peak_idx}, frame={frame_number}")
    ...
    if config_path and peak_frames:
        try:
            ...
            logger.info(
                f"Saved peak frames data with eventfulness and pose vectors to {config_path}")
        except Exception as e:
            logger.error(
                f"Failed to save peak frames data to config.json: {str(e)}")
...
def extract_frames_for_sliding_window(video_path, video_info, interval_seconds=0.5, window_seconds=3):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    frames_dir = os.path.join(
        RESULTS_DIR, f"sliding_window_{video_filename}_{timestamp}")
    os.makedirs(frames_dir, exist_ok=True)
    ...
    while frame_number < frame_count:
        ...
        if ret:
            ...
            # Save the frame
            frame_path = os.path.join(
                frames_dir, f"window_frame_{frame_index}_time_{time_seconds:.2f}.jpg")
            cv2.imwrite(frame_path, frame)
            ...
            if frame_index % 20 == 0:  # Log every 20th frame to avoid log flooding
                logger.info(
                    f"Extracted sliding window frame {frame_index}: time={time_seconds:.2f}s, frame={frame_number}")

            frame_index += 1
        else:
            logger.error(
                f"Failed to extract sliding window frame at time={frame_number/fps:.2f}s, frame={frame_number}")
    ...
    logger.info(
        f"Extracted {len(frames)} sliding window frames to {frames_dir}")
    return frames
...
def perform_pose_estimation(image, draw=True, normalize_centroid=False):
    """
    Performs pose estimation on an image using MediaPipe with reduced face, hand, and foot vectors.

    Args:
        image: OpenCV image (BGR format)
        draw: Boolean indicating whether to draw landmarks on the image (default: True)
        normalize_centroid: Boolean indicating whether to normalize landmarks by centroid (default: False)
            ...
            if draw:
                ...
                for connection in connections:
                    ...
                    if idx1 in LANDMARKS_TO_KEEP and idx2 in LANDMARKS_TO_KEEP:
                        ...
                        cv2.line(annotated_image, (x1, y1),
                                 (x2, y2), (0, 255, 0), 2)
            else:
...
def process_video_with_pose_estimation(video_path, normalize_centroid=True, draw=False, save_frames=False, output_prefix=None, save_numpy=True):
    """
    Processes an entire video, performing pose estimation on each frame with centroid normalization.

    Args:
        video_path: Path to the video file
        normalize_centroid: Whether to normalize landmarks by centroid (default: True)
        draw: Whether to draw pose landmarks on frames (default: False)
        save_frames: Whether to save processed frames to disk (default: False)
        output_prefix: Prefix for output files (optional)
        save_numpy: Whether to save pose vectors as numpy arrays (default: True)

    Returns:
        Dictionary containing:
        - pose_vectors: List of pose vectors for each frame
        - normalized_pose_vectors: List of normalized pose vectors (if normalize_centroid=True)
        - success_frames: List of frame indices where pose estimation was successful
        - frame_count: Total number of frames processed
        - fps: Frames per second of the video
        - success_rate: Percentage of frames with successful pose detection
    ...
    # Create directory for frames if saving is enabled
    frames_dir = None
    if save_frames:
        video_filename = os.path.basename(video_path).split('.')[0]
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        frames_dir = os.path.join(
            RESULTS_DIR, f"pose_estimation_{video_filename}_{timestamp}")
        os.makedirs(frames_dir, exist_ok=True)
        logger.info(f"Saving frames to {frames_dir}")

    # Generate output filenames for numpy arrays
    if save_numpy:
        video_filename = os.path.basename(video_path).split('.')[0]
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        if output_prefix:
            prefix = f"{output_prefix}_{video_filename}_{timestamp}"
        else:
            prefix = f"pose_norm_{video_filename}_{timestamp}"

        pose_vectors_file = os.path.join(
            RESULTS_DIR, f"{prefix}_pose_vectors.npy")
        if normalize_centroid:
            normalized_pose_vectors_file = os.path.join(
                RESULTS_DIR, f"{prefix}_normalized.npy")
    ...
    pose_vectors = []
    normalized_pose_vectors = [] if normalize_centroid else None
    success_frames = []
    ...
    logger.info(
        f"FPS: {fps}, Frame count: {frame_count}, Resolution: {width}x{height}")
    logger.info(
        f"Pose estimation with centroid_normalization={normalize_centroid}, draw={draw}")
    ...
    while True:
        ...
        # Perform pose estimation with centroid normalization and no drawing
        _, pose_vector, pose_success = perform_pose_estimation(
            frame,
            draw=draw,
            normalize_centroid=normalize_centroid
        )

        # Store results
        pose_vectors.append(pose_vector)
        if pose_success:
            success_frames.append(frame_index)
            success_count += 1
    ...
    success_rate = (success_count / frame_index) * \
        100 if frame_index > 0 else 0
    logger.info(f"Pose estimation completed: {frame_index} frames processed")
    logger.info(
        f"Pose detected in {success_count} frames ({success_rate:.1f}% success rate)")

    # Convert to numpy arrays and save
    if save_numpy:
        pose_vectors_np = np.array(pose_vectors)
        np.save(pose_vectors_file, pose_vectors_np)
        logger.info(f"Saved pose vectors to {pose_vectors_file}")

        if normalize_centroid:
            np.save(normalized_pose_vectors_file, pose_vectors_np)
            logger.info(
                f"Saved normalized pose vectors to {normalized_pose_vectors_file}")
    ...
    if save_numpy:
        result['pose_vectors_file'] = pose_vectors_file
        if normalize_centroid:
            result['normalized_pose_vectors_file'] = normalized_pose_vectors_file

    return result

# Wrapper function to run pose estimation pipeline


def run_pose_estimation_pipeline(video_path, output_prefix=None, normalize_centroid=True, draw=False):
    """
    Wrapper function to run the pose estimation pipeline and return results.
    ...
    """
    return process_video_with_pose_estimation(
...
def verify_marker_position(figure, expected_position, expected_value):
    ...
    if 'x' not in line_trace or len(line_trace['x']) != 2:
        logger.error(
            f"Figure verification failed: Invalid line trace structure")
        return False
    ...
    if 'x' not in marker_trace or len(marker_trace['x']) != 1:
        logger.error(
            f"Figure verification failed: Invalid marker trace structure")
        return False
    ...
    if not (line_matches and marker_matches):
        logger.error(
            f"Marker position mismatch: Expected={expected_position}, Line={line_x}, Marker={marker_x}")

    if not value_matches:
        logger.error(
            f"Marker value mismatch: Expected={expected_value:.3f}, Actual={marker_y:.3f}")
...
app.layout.children.append(
    html.Div([
        ...
        html.Pre(id='debug-info', style={'border': '1px solid #ddd',
                 'padding': '10px', 'maxHeight': '200px', 'overflow': 'auto'}),
        html.Div([
            html.Button("Clear Log", id="clear-log-button",
                        style={'margin': '10px'}),
            html.A("Download Log", id="download-log-link", href=f"/download-log",
                   download="sync_debug.log", style={'margin': '10px'})
        ])
    ], style={'marginTop': '30px'})
)

# Callback to handle normalized pose estimation button


@callback(
    Output('normalized-pose-data', 'data'),
    Output('vector-type', 'options'),
    Output('peak-frames', 'data', allow_duplicate=True),
    Input('normalized-pose-button', 'n_clicks'),
    State('current-video', 'data'),
    State('video-info', 'data'),
    State('vector-type', 'options'),
    State('peak-frames', 'data'),
    prevent_initial_call=True
)
def handle_normalized_pose_estimation(n_clicks, video_path, video_info, current_options, peak_frames):
    """
    Process the video with pose estimation and centroid normalization
    when the button is clicked.
    """
    if n_clicks is None or n_clicks == 0:
        return None, current_options, peak_frames
    ...
    try:
        logger.info(
            f"Running pose estimation with normalization on {video_path}")

        # Run the pose estimation pipeline (now integrated in this file)
        results = run_pose_estimation_pipeline(video_path)
        ...
        pose_data = {
            'original_poses_file': results.get('pose_vectors_file', ''),
            'normalized_poses_file': results.get('normalized_pose_vectors_file', ''),
            'frame_count': results['frame_count'],
            'success_rate': results['success_rate']
        }

        logger.info(
            f"Pose estimation completed with {results['success_rate']:.2f}% success rate")

        # Add the normalized pose option to the vector type dropdown if it doesn't exist
        normalized_option = {
            'label': 'Normalized Pose Vector', 'value': 'normalized_pose'}

        # Check if the option already exists
        option_exists = False
        for option in current_options:
            if option['value'] == 'normalized_pose':
                option_exists = True
                break

        # Add the option if it doesn't exist
        if not option_exists:
            current_options.append(normalized_option)

        # If we have peak frames, update them with normalized pose vectors
        updated_peak_frames = peak_frames.copy() if peak_frames else {}

        if updated_peak_frames and results['normalized_poses'] is not None:
            normalized_poses = results['normalized_poses']

            for peak_idx, frame_data in updated_peak_frames.items():
                frame_number = frame_data.get('frame_number', 0)

                # Make sure the frame number is within range
                if 0 <= frame_number < len(normalized_poses):
                    # Add the normalized pose vector to the frame data
                    updated_peak_frames[peak_idx]['normalized_pose_vector'] = normalized_poses[frame_number].tolist(
                    )
                    logger.info(
                        f"Added normalized pose vector to peak frame {peak_idx}, frame {frame_number}")

        return pose_data, current_options, updated_peak_frames
...
# Function to perform pose estimation on the entire video
def perform_full_video_pose_estimation(video_path, video_info, sample_rate=1):
    """
    Performs pose estimation on the entire video, sampling frames at the specified rate.

    Args:
        video_path: Path to the video file
        video_info: Dictionary containing video information
        sample_rate: Sample every Nth frame (default: 1, which processes every frame)
    ...
    while True:
        ...
        if frame_number % sample_rate == 0:
            ...
            _, pose_vector, pose_detected = perform_pose_estimation(
        ...
        if frame_number % 100 == 0:
            logger.info(
                f"Processed {frame_number}/{frame_count} frames ({frame_number/frame_count*100:.1f}%)")
...
# Function to calculate cluster centroids from pose vectors
def calculate_cluster_centroids(peak_frames, cluster_assignments):
    """
    ...
    clusters = {}
    for peak_idx, frame_info in peak_frames.items():
        if peak_idx in cluster_assignments and frame_info.get('pose_detected', False):
            cluster_id = cluster_assignments[peak_idx]
            pose_vector = frame_info.get('pose_vector', None)
...
# Function to compute cosine similarity between pose vectors
def compute_cosine_similarities(pose_data, cluster_centroids):
    """
    ...
    centroid_vectors = np.array([cluster_centroids[cid]
    ...
    for frame_idx, frame_data in pose_data.items():
        ...
        if pose_vector is not None:
            # Reshape pose vector for cosine_similarity function
            pose_array = np.array(pose_vector).reshape(1, -1)

            # Compute cosine similarity with each centroid
            sim_scores = cosine_similarity(pose_array, centroid_vectors)[0]

            # Store similarity scores
            similarities[frame_idx] = {
                'frame_number': frame_data['frame_number'],
                'time': frame_data['time'],
                'similarities': {str(centroid_ids[i]): float(score) for i, score in enumerate(sim_scores)}
            }

    return similarities

# Callback for the full video pose analysis button


@callback(
    [Output('full-video-pose-data', 'data'),
     Output('cluster-centroids', 'data'),
     Output('cosine-similarity-data', 'data'),
     Output('cluster-assignments', 'data', allow_duplicate=True)],
    [Input('run-full-pose-btn', 'n_clicks')],
    [State('current-video', 'data'),
     State('video-info', 'data'),
     State('peak-frames', 'data'),
     State('cluster-assignments', 'data')],
...
def handle_full_video_analysis(n_clicks, video_path, video_info, peak_frames, cluster_assignments):
    ...
    try:
        ...
        # Perform pose estimation on the entire video
        logger.info("Step 1/4: Starting pose estimation on the entire video...")
        pose_data = perform_full_video_pose_estimation(
            video_path, video_info, sample_rate)
        ...
        # If we don't have cluster assignments but we have peak frames, perform clustering
        if peak_frames and not cluster_assignments:
            logger.info(
                "Step 2/4: No existing clusters found. Performing clustering...")
            # Use default max_clusters=6 and vector_type='pose'
            cluster_assignments, cluster_info = cluster_eventfulness_vectors(
                peak_frames, max_clusters=6, vector_type='pose')
        ...
        if peak_frames and cluster_assignments:
            ...
            if pose_data and centroids:
                logger.info("Step 4/4: Computing cosine similarities...")
                similarities = compute_cosine_similarities(
                    pose_data, centroids)
                logger.info(
                    f"Computed cosine similarities for {len(similarities)} frames")
...
# Callback to create a separate cosine similarity graph
@callback(
...
def create_cosine_similarity_graph(similarities, centroids):
    ...
    # Extract frame numbers, times and similarity scores
    frame_numbers = []
    times = []
    ...
    # Create a new figure for cosine similarity
    fig = go.Figure()

    # Add similarity traces to the figure
    for i, (cluster_id, scores) in enumerate(similarity_data.items()):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=scores,
                mode='lines',
                name=f'Cluster {cluster_id} Similarity',
                line=dict(color=color, width=2),
                opacity=0.7,
                hoverinfo='y+x',
                hoverlabel=dict(bgcolor='white')
    ...
    # Update layout
    fig.update_layout(
        title='Cosine Similarity to Cluster Centroids',
...
# Callback to automatically run segmentation after full video analysis


@callback(
    [Output('segmentation-data', 'data', allow_duplicate=True),
     Output('segmentation-graph', 'figure', allow_duplicate=True),
     Output('segmentation-section', 'style', allow_duplicate=True),
     Output('segmentation-info', 'children', allow_duplicate=True)],
    [Input('full-video-pose-data', 'data')],
    [State('video-info', 'data')],
    prevent_initial_call=True
)
def auto_run_segmentation(pose_data, video_info):
    if not pose_data or not video_info:
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update

    # Use default parameters for automatic segmentation
    vector_type = 'pose'
    threshold_factor = 0.05  # Threshold factor for change detection
    min_length = 5
    # Use periodicity-based segmentation by default
    segmentation_method = 'periodicity'

    # Log the start of processing
    logger.info(
        f"Auto-running {segmentation_method}-based time series segmentation with {vector_type} vectors")

    try:
        # Perform segmentation
        segmentation_results = segment_vector_time_series(
            pose_data,
            vector_type=vector_type,
            error_threshold=threshold_factor,
            min_segment_length=min_length,
            segmentation_method=segmentation_method
        )

        if not segmentation_results or not segmentation_results['segments']:
            return dash.no_update, dash.no_update, dash.no_update, dash.no_update
        ...
        # Add eventfulness data if available
        if 'eventfulness_data' in video_info and vector_type in ['eventfulness', 'combined']:
            eventfulness_data = video_info['eventfulness_data']
            x_values = list(range(len(eventfulness_data)))
            fig.add_trace(go.Scatter(
                x=x_values,
                y=eventfulness_data,
                mode='lines',
        ...
        # Add colored regions for each segment
        for i, segment in enumerate(segmentation_results['segments']):
            ...
            fig.add_trace(go.Scatter(
                ...
                name=f'Segment {i+1} Start',
                hoverinfo='text',
                text=f'Segment {i+1} Start: Frame {start_frame} ({start_frame/fps:.2f}s)',
                showlegend=False
            ))

        # Add a final vertical line for the last segment end
        if segmentation_results['segments']:
            last_segment = segmentation_results['segments'][-1]
            color_idx = len(segmentation_results['segments']) % len(
                segment_colors)
            color = segment_colors[color_idx]

            fig.add_trace(go.Scatter(
                x=[last_segment['end_frame'], last_segment['end_frame']],
                y=[0, 1],
                mode='lines',
                line=dict(color=color, width=2, dash='dash'),
                name=f'Segment {len(segmentation_results["segments"])} End',
                hoverinfo='text',
                text=f'Segment {len(segmentation_results["segments"])} End: Frame {last_segment["end_frame"]} ({last_segment["end_frame"]/fps:.2f}s)',
                showlegend=False
            ))
        ...
        # Update layout
        fig.update_layout(
            title=f'Auto {segmentation_method.capitalize()}-Based Time Series Segmentation',
            xaxis=dict(title='Frame Number'),
        ...
        # Create info text
        num_segments = len(segmentation_results['segments'])
        avg_duration = sum(s['duration'] for s in segmentation_results['segments']
                           ) / num_segments if num_segments > 0 else 0

        info_text = [
            html.Div([
                html.Span("Auto Wavelet Segmentation Results: ",
                          style={'fontWeight': 'bold'}),
                f"Found {num_segments} segments using {vector_type} vectors"
            ]),
            html.Div([
                html.Span("Parameters: ", style={'fontWeight': 'bold'}),
                f"Threshold Factor: {threshold_factor}, Min Segment Length: {min_length} frames"
            ]),
            html.Div([
                html.Span("Statistics: ", style={'fontWeight': 'bold'}),
                f"Average Segment Duration: {avg_duration:.1f} frames ({avg_duration/fps:.2f} seconds)"
            ]),
            html.Div([
                html.Span("Method: ", style={'fontWeight': 'bold'}),
                f"{segmentation_method.capitalize()}-based segmentation with data smoothing"
            ])
        ]

        return segmentation_results, fig, {'display': 'block'}, info_text

    except Exception as e:
        logger.error(f"Error in auto segmentation: {str(e)}")
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update

# Callback to download cosine similarity data as CSV


@callback(
    Output("download-cosine-similarity-csv", "data"),
    Input("download-cosine-similarity-button", "n_clicks"),
    [State("cosine-similarity-data", "data"),
     State("cluster-centroids", "data"),
     State("video-info", "data")],
    prevent_initial_call=True
)
def download_cosine_similarity_csv(n_clicks, similarity_data, centroids, video_info):
    if not n_clicks or not similarity_data or not centroids or not video_info:
        return dash.no_update

    import pandas as pd
    import io

    # Extract video filename for the CSV filename
    video_path = video_info.get('video_path', 'video')
    video_filename = os.path.basename(video_path).split('.')[0]
    ...
    sorted_frames = sorted(similarity_data.items(),
                           key=lambda x: similarity_data[x[0]]['frame_number'])

    for frame_idx, frame_data in sorted_frames:
        frame_number = frame_data['frame_number']
        time = frame_data['time']

        # Create a row with frame number and time
        row = {
            'frame_number': frame_number,
            'time_seconds': time
        }

        # Add similarity scores for each cluster
        for cluster_id, score in frame_data['similarities'].items():
            row[f'cluster_{cluster_id}_similarity'] = score
    ...
    return dict(
        content=csv_string,
        filename=f"{video_filename}_cosine_similarity.csv"
    )

# Callback to perform time series segmentation and create visualization


@callback(
    [Output('segmentation-data', 'data'),
     Output('segmentation-graph', 'figure'),
     Output('segmentation-section', 'style'),
     Output('segmentation-info', 'children')],
    [Input('segmentation-button', 'n_clicks')],
    [State('full-video-pose-data', 'data'),
     State('segmentation-vector-type', 'value'),
     State('segmentation-error-threshold', 'value'),
     State('segmentation-min-length', 'value'),
     State('video-info', 'data'),
     State('segmentation-method', 'value')],
    prevent_initial_call=True
)
def handle_segmentation(n_clicks, pose_data, vector_type, error_threshold, min_length, video_info, segmentation_method='wavelet'):
    if not n_clicks or not pose_data or not video_info:
        empty_fig = go.Figure()
        empty_fig.add_annotation(
            text="No segmentation data available. Click 'Run Segmentation' to begin.",
            xref="paper", yref="paper",
            x=0.5, y=0.5, showarrow=False,
            font=dict(size=14, color="gray")
        )
        return None, empty_fig, {'display': 'block'}, "No segmentation performed yet."

    # Log the start of processing
    logger.info(
        f"Starting {segmentation_method}-based time series segmentation with {vector_type} vectors, threshold factor: {error_threshold}, min length: {min_length}")
    ...
    try:
        ...
        if not segmentation_results or not segmentation_results['segments']:
            empty_fig = go.Figure()
            empty_fig.add_annotation(
                text="Segmentation failed or no segments found.",
                xref="paper", yref="paper",
                x=0.5, y=0.5, showarrow=False,
                font=dict(size=14, color="gray")
            )
            return None, empty_fig, {'display': 'block'}, "Segmentation failed or no segments found."
        ...
        # Update layout
        fig.update_layout(
            title='Wavelet-Based Time Series Segmentation',
            xaxis=dict(title='Frame Number'),
            yaxis=dict(title='Value'),
            margin=dict(l=40, r=40, t=40, b=40),
            hovermode='closest',
        ...
        info_text = [
            html.Div([
                html.Span("Wavelet Segmentation Results: ",
                          style={'fontWeight': 'bold'}),
                f"Found {num_segments} segments using {vector_type} vectors"
            ]),
            html.Div([
                html.Span("Parameters: ", style={'fontWeight': 'bold'}),
                f"Threshold Factor: {error_threshold}, Min Segment Length: {min_length} frames"
            ]),
            html.Div([
                html.Span("Statistics: ", style={'fontWeight': 'bold'}),
                f"Average Segment Duration: {avg_duration:.1f} frames ({avg_duration/fps:.2f} seconds)"
            ...
            html.Div([
                html.Span("Method: ", style={'fontWeight': 'bold'}),
                f"Wavelet-based segmentation with smoothing"
            ])
        ]
        ...
        logger.error(f"Error in wavelet segmentation: {str(e)}")
        empty_fig = go.Figure()
        empty_fig.add_annotation(
            text=f"Error during wavelet segmentation: {str(e)}",
            xref="paper", yref="paper",
            x=0.5, y=0.5, showarrow=False,
            font=dict(size=14, color="red")
        )
        return None, empty_fig, {'display': 'block'}, f"Error: {str(e)}"
...
def create_cosine_similarity_graph(similarities, centroids):
    ...
    # Update legend
    updated_figure['layout']['legend'] = dict(
        orientation='h',
        yanchor='bottom',
        y=1.02,
        xanchor='right',
